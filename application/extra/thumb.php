...
function get_cover($cover_id, $field = null)
{
    ...
    if ($picture === false) {
        ...
        $picture = $pictureModel->where(['status' => 1,'id'=>$cover_id])->find()->toArray();
        cache($tag, $picture);
    }
...
function getThumbImageById($cover_id, $width = 100, $height = 'auto', $type = 0, $replace = false)
{
    ...
    if (empty($picture)) {
        $pictureModel = new PictureModel();
        $picture = $pictureModel->where(['status' => 1,'id'=>$cover_id])->find();
...
function get_url_with_domain($path)
{
    ...
    if ($not_http_remote && $not_https_remote) {
        ...
        $path = 'http://' . str_replace(['////', '///', '//'], '/', $_SERVER['HTTP_HOST'] . '/' . $root . '/' . $path);
