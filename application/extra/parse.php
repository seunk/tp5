...
function parse_popup($content)
{
    ...
    foreach ($img_tag[0] as $k => &$v) {
        $content = str_replace($v, '<a class="popup" href="' . $img_src[1][$k] . '" title="'.lang('_CLICK_TO_SEE_THE_BIG_PICTURE_').'">' . $v . '</a>', $content);
    }
...
{
    $contentHandlerModel = new \app\common\model\ContentHandlerModel();
    $content = $contentHandlerModel->renderVideo($content);
    return $content;
}
...
function filter_content($content){
    $contentHandlerModel = new \app\common\model\ContentHandlerModel();
    $content = $contentHandlerModel->filterHtmlContent($content);
    return $content;
}