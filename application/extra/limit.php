...
{
    $obj = new \app\common\model\ActionLimitModel();

    $item = ['action' => $action, 'model' => $model, 'record_id' => $record_id, 'user_id' => $user_id, 'action_ip' => $ip];
    if(empty($record_id)){
...
function get_time_unit($key = null){
    $array = ['second' => lang('_TIME_SECOND_'), 'minute' => lang('_TIME_MINUTE_'), 'hour' => lang('_HOUR_'), 'day' => lang('_TIME_DAY_'), 'week' => lang('_TIME_WEEK_'), 'month' => lang('_TIME_MONTH_'), 'year' => lang('_TIME_YEAR_')];
...
function get_action_name($key){
    !is_array($key) && $key = explode(',',str_replace(['[',']'],'',$key));
    $return =[];
    foreach($key as $val){
