...
class ContentHandlerModel
{
    ...
    public function handleAtWho($content, $url = '', $args = array(), $app_name = '', $escap_first = false)
    {
        ...
        foreach ($uids as $uid) {
            ...
            $message = lang('_COMMENTS_WITH_COLON_') . mb_substr(op_t($content), 0, 50, 'utf-8');
            if ($url == '') { //如果未设置来源的url，则自动跳转到来源页面
                $url = $_SERVER['HTTP_REFERER'];
            }
            $messageModel = new MessageModel();
            $messageModel->sendMessage($uid, $title, $message, $url, $args);
        }
    }
    ...
    {
        $return = cache('video_info_' . md5($link));
        if (empty($return)) {
            vendor('collection.phpQuery');
...
//                preg_match("/[0-9]*/", $id_bi, $cid_bi);
            ...
            cache('video_info_' . md5($link), $return, 60 * 60);
        }
        return $return;
    }
