...
class PictureModel extends BaseModel{
    ...
    public function upload($files, $setting, $driver = 'local', $config = null){
        ...
        foreach ($files as $key => $file) {
            ...
            if(in_array($ext, ['jpg','jpeg','bmp','png'])){
        ...
        if($info){ //文件上传成功，记录文件信息
            foreach ($info as $key => &$value) {
                ...
                if($this->allowField(true)->save($value)){
                    $value['id'] = $this->id;
                } else {
    ...
    private function _dealPicture($temp_name)
    {
        ...
        if ($waterOpen) {
            ...
            if ($waterImage) {
                ...
                $water = './public/backstage/images/water.png';
            }
            $water_open = file_exists($water);
            if ($water_open) {
                $position=modC('WATER_SPACE',0,'picture');
                $watermark = new service\WaterMarkService($temp_name, $position);
    ...
    public function download($root, $id, $callback = null, $args = null){
        ...
        switch ($file['location']) {
            ...
            case 1: //TODO: 下载远程FTP文件
                break;
            default:
                $this->error = lang('_UNSUPPORTED_FILE_STORAGE_TYPE_WITH_EXCLAMATION_');
                return false;
    ...
    public function getPictureList($page=1,$r=10)
    {
        ...
        $list=$this->where($map)->page($page,$r)->order('create_time desc')->select()->toArray();
