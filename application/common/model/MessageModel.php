...
class MessageModel extends BaseModel
{
     ...
     * @return bool
     */
    public function sendMessage($to_uids, $title = '您有新的消息', $content = '', $url = '', $url_args = [], $from_uid = -1, $type = 'Common_system', $tpl = '')
    {
        $to_uids = is_array($to_uids) ? $to_uids : [$to_uids];
     ...
     * @return bool
     */
    public function sendMessageWithoutCheckSelf($to_uids, $title = '您有新的消息', $content = '', $url = '', $url_args = [], $from_uid = -1, $type = 'Common_system', $tpl = '')
    {
        $to_uids = is_array($to_uids) ? $to_uids : [$to_uids];
        $to_uids=$this->_removeOldUser($to_uids);
        if(!count($to_uids)){
    ...
    public function sendEmail($to_uids, $title = '您有新的消息', $content = '', $url = '', $url_args = [], $from_uid = -1, $type = 'Common_email', $tpl = '')
    {
        $from_uid == -1 && $from_uid = is_login();
        $message_content_id = $this->addMessageContent($from_uid, $title, $content, $url, $url_args, $type);
        $to_uids = is_array($to_uids) ? $to_uids : [$to_uids];
        ...
        do {
            ...
            foreach ($do_to_uids as $to_uid) {
                ...
                if(!empty($aUrl)){
                    $newUrl='http://' . $_SERVER['HTTP_HOST'] .url($url);
    ...
    public function sendMobileMessage($to_uids, $title = '您有新的消息', $content = '', $url = '', $url_args = [], $from_uid = -1, $type = 'Common_mobile', $tpl = '')
    {
        $from_uid == -1 && $from_uid = is_login();
        $message_content_id = $this->addMessageContent($from_uid, $title, $content, $url, $url_args, $type);
        $to_uids = is_array($to_uids) ? $to_uids : [$to_uids];
     ...
     * @return bool
     */
    public function sendALotOfMessageWithoutCheckSelf($to_uids, $title = '您有新的消息', $content = '', $url = '', $url_args = [], $from_uid = -1, $type = 'Common_system', $tpl = '')
    {
        $to_uids = is_array($to_uids) ? $to_uids : [$to_uids];
        $to_uids=$this->_removeOldUser($to_uids);
        if(!count($to_uids)){
    ...
    private function _removeOldUser($to_uids)
    {
        $map['uid']=['in',$to_uids];
        $map['status']=1;
        $map['last_login_time']=['gt',get_time_ago('month')];
        $member = new MemberModel();
        $uids=$member->where($map)->field('uid')->select();
    ...
    private function _initUserMessageSession($uids, $type)
    {
        ...
        foreach ($need_uids as $val) {
            cache('MY_MESSAGE_SESSION_' . $val, null);
            $dataList[] = ['uid' => $val, 'type' => $type, 'status' => 1];
        }
        unset($val);
        if (count($dataList)) {
            $messageTypeModel->saveAll($dataList);
        }
        return true;
    }
    ...
    private function addMessageContent($from_uid, $title, $content, $url, $url_args, $type)
    {
        ...
        $message_id = db('message_content')->update($data_content);
        return $message_id;
    }
    ...
    public function getContent($id)
    {
        ...
        if (empty($content)) {
            $content = db('message_content')->find($id);
            if ($content) {
                ...
                if (in_array($content['type'], ['1', '2', '3', '0', '4', '5'])) {
                    $content['type'] = 'Common_system';
                }
                if ($content['from_id']) {
                    $content['user'] = query_user(['nickname', 'avatar64', 'uid'], $content['from_id']);
                }
            }
    ...
    public function getHaventReadMessageCount($uid)
    {
        ...
        $map['to_uid']=$uid;
        $map['is_read']=0;
        $map['type']=['notlike',['%Common_email%','%Common_mobile%'],'AND'];
    ...
    {
        $messages = $this->where('to_uid=' . $uid . ' and  is_read=0  and last_toast=0')->order('id desc')->limit(99999)->select();
    ...
    {
        $now = time();
        $this->where('to_uid=' . $uid . ' and  is_read=0 and last_toast=0')->setField('last_toast', $now);
    }
    ...
    public function getAllMessageSession()
    {
        ...
        if ($message_session === false) {
            ...
            foreach ($message_session as &$val) {
                ...
                $val['module'] = 'Common';
                $val['logo'] = __ROOT__ . '/static/images/message_logo/' . $val['logo'];
                !isset($val['sort']) && $val['sort'] = 0;
                $val['alias'] = '系统';
            }
            unset($val);

            $model_message_session = [];
            $module_alias = [];
            ...
            foreach ($modules as $val) {
                ...
                if (!in_array($val['name'], ['Core'])) {
                    $addArray = [['name' => '', 'title' => $val['alias'] . '消息', 'logo' => NULL]];
                    if (count($conf)) {
            ...
            foreach ($model_message_session as $key => $val) {
                ...
                foreach ($val as $one_type) {
                    ...
                    if ($one_type['logo'] == null) {//模块会话使用模块名logo作为会话logo
                        $one_type['logo'] = __ROOT__ . '/static/images/message_logo/' . strtolower($key) . '.png';
                    } else {
                        $one_type['logo'] = __ROOT__ . '/static/images/message_logo/' . $one_type['logo'];
                    }
    ...
    public function getMyMessageSession($uid = 0)
    {
        ...
        if ($message_session === false) {
            $map['uid'] = $uid;
            $map['status'] = 1;
            $message_session = db('MessageType')->where($map)->select();
            if(!count($message_session)){//默认所有用户拥有系统会话
                $message_session[]=['uid'=>$uid,'type'=>'Common_system','status'=>1];
            }
            cache($tag, $message_session);
        }
        return $message_session;
    ...
    private function _initMessage(&$messages)
    {
        foreach ($messages as &$v) {
            if (in_array($v['type'], ['1', '2', '3', '0', '4', '5'])) {
    ...
    {
        $content_id=db('message_content')->where(['title'=>'话题通知','content'=>'您关注的#'.$content.'#话题已更新。','url'=>'home/Topic/index'])->field('id')->select();
         foreach ($content_id as $cid){
             $cids[]=$cid['id'];
         }
        $map['content_id']=['in',$cids];
         $data=model('Message')->where($map)->select();
        if(!$data){
            $readUids=$uids;
        }else{
            foreach ($data as $vo)
    ...
    public function getSessionMessage($session, $start = 0, $num = 5, $uid = 0)
    {
        ...
        $map['to_uid'] = $uid;
        $map['type'] = $session;
        $messages = db('MessageOld')->where($map)->order('id desc')->limit($start, $num)->select();
        $this->_initMessage($messages);
        return $messages;
    }
    ...
    public function getSessionMessageFirst($type = 'Common_system', $uid)
    {
        ...
        if (count($unreadMessageList)) {
            ...
            if (count($readMessageList)) {
                $messageList = array_merge($unreadMessageList, [['line' => 1]], $readMessageList);
    ...
    private function _transferMessage($uid,$message_session='Common_system')
    {
        ...
        if(count($dataList)){
            $res=db('MessageOld')->insertAll($dataList);
            if($res){
                $ids=array_column($dataList,'id');
                $map_delete['id']= ['in',$ids];
                $this->where($map_delete)->delete();
            }
        }

        //移动一个月未登录用户的消息，并设为已读
        $time=cache('last_message_old_time');
        if($time==null||$time<get_time_ago('day',10)){
            ...
            $map_uid['to_uid']= ['not in',$uids];
            do{
                $oldDataList=$this->where($map_uid)->limit(999)->select();
                if(count($oldDataList)){
                    $messageOldModel=db('MessageOld');
                    $res=$messageOldModel->insertAll($oldDataList);
                    if($res){
                        cache('last_message_old_time',time());
                        $messageOldModel->where(['is_read'=>0])->setField('is_read',1);
