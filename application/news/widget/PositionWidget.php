...
class PositionWidget extends Controller{
    ...
    /* 显示指定分类的同级分类或子分类列表 */
    public function lists($position=4,$category=0,$limit=5,$filed='*'){
        /**
         * 获取推荐位数据列表
        ...
        $lists=$newsModel->position($position,$category,$limit,$filed);
        foreach($lists as &$val){
            $val['user']=query_user(['space_url','nickname'],$val['uid']);
        }
        $this->assign('lists', $lists);
        return $this->fetch(T('application://news@widget/position'));
    }
