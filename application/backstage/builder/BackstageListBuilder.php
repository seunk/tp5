...
class BackstageListBuilder extends  BackstageBuilder{
    ...
    {
        return $this->button(lang('_CLEAR_OUT_'), ['class' => 'layui-btn layui-btn-primary ajax-post tox-confirm', 'data-confirm' => lang('_CONFIRM_CLEAR_OUT_'), 'url' => url('', ['model' => $model]), 'target-form' => 'ids', 'hide-data' => 'true']);
    }
        ...
        $this->convertKey('multiImage', 'html', function ($value, $key, $item) {
            ...
            if($value){
                ...
                foreach($value as $val){
                    ...
                    $sc_src = $sc_src == '' ? $src : $sc_src;
                    $html .= "<a title=\"" . lang('_VIEW_BIGGER_') . "\" href=\"$sc_src\" style='margin-right:5px;'><img src=\"$sc_src\"/ style=\"width:80px;height:80px\"></a>";
                }
            }
            $html.="</div>";
            return $html;
    ...
    public function clearTrash($model = '')
    {
        if (Request()->isPost()) {
            if ($model != '') {
                ...
                if ($result) {
                    $this->success(lang('_SUCCESS_TRASH_CLEARED_', ['result' => $result]));
                }
