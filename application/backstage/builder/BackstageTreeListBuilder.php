...
class BackstageTreeListBuilder extends BackstageBuilder
{
    ...
    {
        $map = [-1 => lang('DELETE'), 0 => lang('DISABLE'), 1 => lang('ENABLE'), 2 => '未审核'];
        return $this->key($name, $title, 'status', $map);
    }

    public function keyYesNo($name, $title)
    {
        $map = [0 => lang('_NOT_'), 1 => lang('YES')];
    ...
    /**
     * @param $pattern url函数解析的URL字符串，例如 Backstage/Test/index?test_id=###
     * ###将被id替换
     * @return callable
     */
