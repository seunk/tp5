...
class UploadavatarWidget extends Controller
{
    ...
    public function render($uid = 0)
    {
        ...
        $this->assign('uid', $uid);
        return $this->fetch('widget/uploadavatar');
    }


    public function getAvatar($uid = 0, $size = 256)
    {
        $avatar = db('avatar')->where(['uid' => $uid, 'status' => 1, 'is_temp' => 0])->find();
        if ($avatar) {

            if($avatar['driver'] == 'local'){
                $avatar_path = "/upload/avatar".$avatar['path'];
            ...
            $role_id=$memberModel->where('uid='.$uid)->field('show_role')->select();
    ...
    public function cropPicture($crop = null,$path)
    {
        ...
        if (strtolower($driver) == 'local') {
            ...
            //返回新文件的路径
            return  cut_str('/upload/avatar',$path,'l');
        }else{
