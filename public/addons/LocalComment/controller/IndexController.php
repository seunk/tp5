...
class IndexController extends Controller
{
    ...
    {
        $this->commentModel = model('Addons://LocalComment/LocalComment');
    }
    ...
    public function addComment()
    {
      ...
      // $uid=is_login();
        $aPath = input('post.path', '', 'urldecode');
        $aPath = explode('/', $aPath);
        $aApp = $aPath[0];
        $aMod = $aPath[1];
        $aRowId = $aPath[2];
       ...
       /* $map['app']=$aApp;
        ...
        $aUrl = input('post.this_url', '', 'text');
        $aExtra = input('post.extra','','text');
        parse_str($aExtra);
        ...
        $aCountModel = input('count_model', '', 'text');
        $aCountField = input('count_field', '', 'text');

        $aContent = input('content', '', 'text');
        $aUid = input('uid', '', 'intval');
        if (empty($aContent)) {
            $this->error('评论内容不能为空');
        }
        ...
        if ($res) {
            ...
            db($aCountModel)->where(['id' => $aRowId])->setInc($aCountField);
            ...
            if (!is_login()) {
                if ($aUid) {
                    ...
                    $messageModel->sendMessage($aUid, $title, $message, $aUrl, [$field => $aRowId], 0);
                }
                $result['status'] = 1;
                $result['data'] = $html;
                $result['info'] = '评论成功';
            ...
            if ($aUid) {
                $user = query_user(['nickname', 'uid'], is_login());
                $title = $user['nickname'] . '评论了您';
                $message = '评论内容：' . $aContent;
                $messageModel->sendMessage($aUid, $title, $message, $aUrl, [$field => $aRowId]);
            ...
            $uids = array_subtract($uids, [$aUid]);
            foreach ($uids as $uid) {
                $user = query_user(['nickname', 'uid'], is_login());
                $title = $user['nickname'] . '@了您';
                $message = '评论内容：' . $aContent;

                $messageModel->sendMessage($uid, $title, $message, $aUrl, [$field => $aRowId]);
            }
            $result['status'] = 1;
            $result['data'] = $html;
            $result['info'] = '评论成功';
    ...
    {
        $aApp = input('post.app', '', 'text');
        $aMod = input('post.mod', '', 'text');
        $aRowId = input('post.row_id', '', 'intval');
        $aPage = input('post.page', '', 'intval');
        $count = modC($aMod . '_LOCAL_COMMENT_COUNT', 10, $aApp);
        $commentModel = $this->commentModel;

        $param['where'] = ['app' => $aApp, 'mod' => $aMod, 'row_id' => $aRowId, 'status' => 1];
        $param['page'] = $aPage;
        $param['count'] = $count;
        ...
        $html .= '<div class="pager">' . getPageHtml('local_comment_page', $pageCount, ['app' => $aApp, 'mod' => $aMod, 'row_id' => $aRowId], $aPage) . '</div>';
    ...
    public function deleteComment()
    {
        ...
        if ($result) {
            db($aCountModel)->where(['id' => $comment['row_id']])->setDec($aCountField);
