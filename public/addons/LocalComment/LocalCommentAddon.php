...
class LocalCommentAddon extends Addon
{
    ...
    public function install()
    {
        ...
        db()->execute("DROP TABLE IF EXISTS `{$prefix}local_comment`");
        db()->execute(<<<SQL
...
SQL
    ...
    public function localComment($param)
    {
        ...
        //增加用户信息
        foreach ($list as &$e) {
            $e['user'] = query_user(['uid', 'avatar64', 'nickname', 'space_url'], $e['uid']);
        }
        unset($e);
        $pageCount = ceil($total_count / $count);
       $pageHtml = getPageHtml('local_comment_page',$pageCount,['app'=>$app,'mod'=>$mod, 'row_id'=>$row_id],1);
        //显示页面
        ...
        $this->assign($param);
        $this->assign('myInfo',query_user(['avatar64','nickname','uid','space_url'],is_login()));
        echo $this->fetch('comment');
    }

    private function getCommentModel()
    {
        $objModel = get_Addons_model('LocalComment');
    ...
    {
        $model = $this->getCommentModel();
        $map = ['app' => $app, 'mod' => $mod, 'row_id' => $row_id, 'status' => 1];
        $param['where'] = $map;
        $param['page'] = $page;
        $param['count'] = $count;
